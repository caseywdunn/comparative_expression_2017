...
```{r summary, echo=FALSE, message=FALSE}
	...
	phase_levels = c( "Early", "Mid", "Late" )

	Dm = D %>% 
		gather( Early, Mid, Late, key="phase", value="similarity" ) %>%
	...
	pa = Dm %>% 
		ggplot( aes(x=phase, y=similarity) ) +
	    ...
	    geom_jitter(shape=16, position=position_jitter(0.2), alpha=alpha, color=color) + 
			coord_flip() + 
			ggtitle("(a) Original, with ctenophore in red")
	...
	pb = Dm_no_ct %>% 
	    ...
	    geom_jitter(shape=16, position=position_jitter(0.2), alpha=0.4) + 
			coord_flip() + 
			ggtitle("(b) New, without ctenophore")
...
There were `r nrow(Phase)` gene in the original data file. `r sum(n_species_sampled == 0)` of these had no phase annotations for any species. `r nrow(phase_sub)` had phase annotations for 2 or more species.
