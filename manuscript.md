...
All files needed to re-execute the analyses presented in this document are available at <https://github.com/caseywdunn/comparative_expression_2017>. The most recent commit at the time of the analysis presented here was a36c824180ff007a8e6f65d305834f7ce9bc6b42. See the `readme.md` file in this repository for more information on the contents of the source file and how to re-execute them.
...
The speciation nodes in the gene tree were then calibrated to the dates specified in the KMRR code. This was necessary to scale the branch lengths so that they are equivalent across the trees, and in turn to scale the phylogenetic independent contrasts (which take branch length into account) so they are equivalent across trees. We used the `chronos()` function from the R package ape<sup>36</sup> for this scaling, with the `correlated` model. See supplementary Materials for additional sensitivity analyses to time calibration. Some trees could not be calibrated with these hard node constraints, and were discarded.
