...
```{r pairwise, echo=FALSE, cache=TRUE, message=F, warning=F}
	get_pairwise_summary = function ( nhx ) {
		...
		D %<>% left_join(
			nhx@data %>%
				select( D, node, node_age, label, pic ),
			by = c( "mrca" = "node")
	...
	pairwise_summary$Event = NA
	pairwise_summary$Event[ pairwise_summary$D == "N" ] = "Speciation"
	pairwise_summary$Event[ pairwise_summary$D == "Y" ] = "Duplication"
	...
	calibration_times_all = data.frame(
        age =
	...
	pairwise_summary %>% 
		...
		rename( clade = label ) %>% 
		left_join( calibration_times_all, clade=clade ) %>%
		ggplot() + 
...
There has been considerable recent intrest in, and controversy about, the ortholog conjecture [@Koonin:2005fp; @Studer:2009bm; @Nehrt:2011bm; @Gabaldon:2013fi]. Our results are consistent with multiple studies that have not found support for the ortholog conjecture [@Studer:2009bm; @Nehrt:2011bm; @Yanai:2004hk]. Another sudy found only slightly greater similarity between orthologs than paralogs [@Altenhoff:2012ea].
...
The ortholog conjecture does not have to be an all or nothing question. It may be the case that the rates of phenotypic evolution following duplication may be greater than that following duplication in some organisms, gene families, and evolutionary processes. We just don't find evidence for it when summarizing gene expression acriss tissues with Tau in these mammals. This calls into question the general predictive power of the ortholog conjecture, and until these processes are better understood it will be necessary to test for it in each situation. These tests should be articulated in terms of clear alternative hypotheses [@Studer:2009bm] that make distinct phylogenetic comparative predictions.
