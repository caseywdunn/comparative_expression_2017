...
```{r simulations_null, cache=TRUE, echo=FALSE, fig.cap="XX_PIC_VAR", warning=FALSE, message=FALSE}
	...
	sim_tau = function( nhx ) {
		tau_original = nhx@data$Tau [ 1:length(nhx@phylo$tip.label) ]
		names( tau_original ) = nhx@phylo$tip.label
		brownian_model = fitContinuous( nhx@phylo, tau_original )
		x = fastBM( 
				nhx@phylo, 
	...
	gene_trees_sim_null = lapply( gene_trees_calibrated[1:1000], sim_tau )
	
	# Perform contrasts 
	gene_trees_sim_null %<>% add_pics_to_trees()
	
	# Summarize results
	nodes_sim_null_contrasts = gene_trees_sim_null %>% summarize_contrasts()
	
	# Plot
	g_phylo_null = 
		nodes_sim_null_contrasts %>% 
		ggplot( aes( x=pic, y=..density.., col=Event ) ) + 
			geom_freqpoly( binwidth=.001, position="identity" ) +
			xlab( "Phylogenetic independent \ncontrasts (PIC)" ) +
			ylab( "Density" ) +
			xlim( 0, 0.075 ) + 
			theme_classic() + 
			ggtitle( "(b) Phylogenetic result under null simulation" )
	...
	g_pair_null = 
		...
		ggplot() + 
			geom_point( aes( x=age, y=rho, col=Event, size=n ) ) +
			theme_classic() +
			ggtitle( "(a) Pairwise result under null simulation" )
