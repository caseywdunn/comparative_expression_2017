...
In order to better understand why our phylogenetic analysis seem to support a different conclusion (*i.e.*, no support for the ortholog conjecture) than the published analysis of KMRR [@KryuchkovaMostacci:2016iw] (*i.e.*, strong support for the ortholog conjecture), we first checked to make sure we could reproduce their specific result. This is important since we are only looking at at a subset of the data they considered, the Brawand *et al.* 2011 [@Brawand:2011du] dataset. We can replicate their result - we find a higher tau correlation between orthologs (R=`r round( ortholog_r, 3) `) then between paralogs (R=`r round( paralog_r, 3)`).
