...
```{r phylogenetic_signal, cache=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
...
We removed the `r k_percentile` of genes with the lowest phylogenetic signal, corresponding to a K cutoff for retained genes of greater than `r signif( k_thresh, 3)`. Repeating the phylogenetic analyses on this subset of genes with strongest phylogenetic signal for Tau still do not reject the null hypothesis (Wilcoxon p = $`r  wilcox_test_result_k %>% round( 3 )`$). 


```{r model_selection, cache=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
...
Of the `r nrow(tree_summary)` genes considered, and a $\Delta$AIC threshold of `r dAIC_thresh`, `r sum(tree_summary$dAIC_bm < dAIC_thresh & tree_summary$dAIC_ou >= dAIC_thresh)` have $\Delta$AIC less than the threshold only for Brownian Motion (BM), `r sum(tree_summary$dAIC_bm >= dAIC_thresh & tree_summary$dAIC_ou < dAIC_thresh)` have $\Delta$AIC less than the threshold only for Ornstein Uhlenbeck (OU), and `r sum(tree_summary$dAIC_bm < dAIC_thresh & tree_summary$dAIC_ou < dAIC_thresh)` have $\Delta$AIC less than the threshold for both models. That gives a total of `r sum( tree_summary$dAIC_bm < dAIC_thresh )` genes that have $\Delta$AIC less than the threshold for BM and `r sum( tree_summary$dAIC_ou < dAIC_thresh )` genes that have $\Delta$AIC less than the threshold for OU. 

Repeating the BM phylogenetic analyses on the subset of `r sum( tree_summary$dAIC_bm < dAIC_thresh )` genes that have $\Delta$AIC less than the threshold for BM still does not reject the null hypothesis (Wilcoxon p = $`r  wilcox_test_result_bm %>% round( 3 )`$).

```{r model_ou, cache=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r Tau_v_max, cache=TRUE, echo=FALSE, warning=FALSE, fig.width=3.42, fig.height=3.42, message=FALSE, fig.cap="Genes with higher maximum observed expression tend to have lower Tau. The blue line is the linear model for the relationship between the two."}
