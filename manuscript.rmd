...
Here we reconsider two recent comparative analyses of gene expression data [@KryuchkovaMostacci:2016iw; @Levin:2016bp]. 
...
```{r load_expression, message=F, warning=F}
	expression = lapply(
...
```{r load_trees, cache=TRUE, message=F, warning=F}
...
There were `r length(gene_trees)` parsed.


```{r add_expression_to_trees, cache=TRUE, message=F, warning=F}
...
```{r prep_trees, cache=TRUE, message=F, warning=F}
...
`r length(gene_trees_sub)` trees passed taxon sampling criteria.

`r length(gene_trees_calibrated)` were time calibrated.

```{r contrasts, cache=TRUE, message=F, warning=F}
...
```{r show_result, cache=TRUE, message=F, warning=F}
...
```{r pairwise, cache=TRUE, message=F, warning=F}
